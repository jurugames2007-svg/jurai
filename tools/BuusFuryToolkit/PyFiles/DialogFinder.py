...
class BuusFuryEditor:
    def __init__(self, root):
        self.root = root
        self.root.title("Buu's Fury Studio (Pro Edition)")
        self.root.geometry("1000x700")  # Made slightly wider to accommodate the resizable panels

        self.text_entries = []
        self.display_mapping = {}  # Maps the UI listbox index to the master list index
        self.table = {}
        self.reverse_table = {}
        self.rom_path = ""
        ...
        def save_current_string(end_index):
            ...
            if in_string:
                ...
                if readable_chars >= 2 and (vowels > 0 or is_ui) and (brackets <= readable_chars + 2):
                    ...
                    # Count consecutive 00 00 pairs after the terminator.
                    # These are bytes freed by previous shorter edits and are
                    # available for expansion (the original game terminator
                    # sitting beyond the padding is also counted, since saving
                    # text that fills all max_bytes leaves it untouched).
                    free_bytes = 0
                    j = end_index + 2  # skip the 00 00 terminator itself
                    while j + 1 < len(data) and data[j] == 0x00 and data[j + 1] == 0x00:
                        free_bytes += 2
                        j += 2
    ...
    def populate_list(self, query=""):
        """Populates the listbox, applying the search filter if one exists."""
        self.listbox.delete(0, tk.END)
        self.display_mapping.clear()

        list_index = 0

        # Only show the success message if there is no active search
        if not query:
            self.listbox.insert(tk.END, f"--- Scan Complete: Found {len(self.text_entries)} clean strings ---")
            self.listbox.itemconfig(list_index, {'fg': 'green'})
            list_index += 1

        query = query.lower()

        for original_idx, entry in enumerate(self.text_entries):
            gba_ptr = entry['address'] + 0x08000000
            ptr_hex = f"{gba_ptr:08X}"

            # Filter logic: Check if query is in the text OR in the address pointer
            if query in entry['text'].lower() or query in ptr_hex.lower():
                preview_text = entry['text'].replace('\n', ' ↵ ')
                self.listbox.insert(tk.END, f"[{ptr_hex}] {preview_text}")

                # Map the UI position to the master list position
                self.display_mapping[list_index] = original_idx
                list_index += 1
    ...
    def on_select(self, event):
        ...
        # If the user clicked a status message, ignore it
        if index not in self.display_mapping:
            return

        actual_index = self.display_mapping[index]
        self.current_entry = self.text_entries[actual_index]
    ...
    def save_to_rom(self):
        ...
        try:
            ...
            self.populate_list(self.search_var.get())  # Refresh list but maintain search filter!
    ...
    def setup_ui(self):
        ...
        # 2. Main Area (Using PanedWindow for resizable split)
        main_pane = tk.PanedWindow(self.root, orient=tk.HORIZONTAL, sashrelief=tk.RAISED, sashwidth=6)
        main_pane.pack(side=tk.BOTTOM, fill=tk.BOTH, expand=True, padx=10, pady=10)

        # --- LEFT PANEL (List & Search) ---
        left_frame = tk.Frame(main_pane)
        main_pane.add(left_frame, minsize=350)  # Prevents collapsing it too small
        ...
        self.search_var.trace("w", lambda name, index, mode: self.populate_list(self.search_var.get()))
        ...
        # Editor Header & Zoom Controls
        header_frame = tk.Frame(right_frame)
        header_frame.pack(side=tk.TOP, fill=tk.X, pady=(0, 5))
        tk.Label(header_frame, text="Edit Selected Text:").pack(side=tk.LEFT)
        tk.Button(header_frame, text="A+", command=self.zoom_in, width=3).pack(side=tk.RIGHT, padx=2)
        tk.Button(header_frame, text="A-", command=self.zoom_out, width=3).pack(side=tk.RIGHT)

        self.text_editor = tk.Text(right_frame, height=12, width=40, font=("Courier", self.editor_font_size))
        self.text_editor.pack(fill=tk.BOTH, expand=True, pady=5)  # Now expands to fill vertical space!
